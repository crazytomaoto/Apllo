...
import com.hysd.android.platform_ub.net.base.IRequestCallBack;
...
import com.hysd.android.platform_ub.net.http.HttpRequest;
...
//                MessageCenter.getInstance().sendEmptyMesage(GlobalMessageType.CommonMessageType.USER_TOKEN_EXPIRE);
    ...
    public final void exec(ResponseDataType.HttpMethod method) {
        //重试执行重新new处理
...
//		request.setUrl(PlatformConfig.getString(PlatformConfig.SERVICES_URL) + getTypeURL());
        ...
        try {
            ...
            String languange = PlatformConfig.getString(Constant.SpConstant.USER_LANGUANGE);
            if (!BeanUtils.isEmpty(languange)) {
                request.addHttpHeader("l", languange);
            }
...
//        request.addHttpHeader("devType", "0");
        ...
        request.setEncode("UTF-8");
        request.setTimeout(Constant.TIME_OUT_INT);
        //请求的基本地址
