 ...
 * NBO矿池
 */
public class MiningPoolNBOAcy extends BasicActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_layout_ming_nbo);
    ...
    protected void handleStateMessage(Message message) {
        switch (message.what) {
            ...
            case GlobalMessageType.RequestCode.OREPOOL:
                        ...
                        if (null != listBeans && listBeans.size() > 0) {
                            ...
                            if (newList.size() > 1) {
                                try {
                                    ...
                                    lineChartManager1.showLineChart(this, newList, "", getResources().getColor(R.color.color_purple3));
                ...
                if (state != 2) {
                    iRequest.getBalanceOfNbo(DaoUtil.selectNowWallet().getAddress(), false);
                } else {
                    hud.dismiss();
                    refreshLayout.complete();
                }
    ...
    private void initView() {
        tvTitle.setText(R.string.tip214);
        tvWhat.setText(getString(R.string.tip213) + Constant.TYPE_NBO);
        tvTheme.setText(getString(R.string.tip211) + "(" + Constant.TYPE_NBO + ")");
        listNotice = new ArrayList<>();
        final GifDrawable gifDrawable = (GifDrawable) ivInvite.getDrawable();
        gifDrawable.start();
        initLoading(this);
        hud.show();
        iRequest.getPollAddressState(Constant.TYPE_NBO, DaoUtil.selectNowWallet().getAddress());
        iRequest.queryNotice(1, 10, "", 2);
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.iv_detail:
                ...
                UiHelper.startProfitDetailAcy(this, Constant.TYPE_NBO);
                break;
            case R.id.rl_all_notice:
                if (ClickUtil.isNotFirstClick()) {
                    return;
                }
                UiHelper.startNoticeAllAcy(this, 2);
                break;
            case R.id.ll_active_miner:
                ...
                if (state == 1) {
                    if (Double.parseDouble(balanceBean.getBalance()) < 8) {
                        ToastUtil.show(this, getString(R.string.tip225));
                        return;
                    } else {
                        DialogUtil.dialogActiveNbo(this, Constant.TYPE_NBO, new IPassGetInput() {
                            @Override
                            public void getTextString(String pass) {
                                if (pass.equals(PlatformConfig.getString(Constant.SpConstant.WALLET_PASS))) {
                                    ...
                                    ToastUtil.show(MiningPoolNBOAcy.this, getString(R.string.error_pwd));
                                }
                            }
            ...
            case R.id.ll_miner_power:
                ...
                UiHelper.startMiningPowerAcy(this, Constant.TYPE_NBO);
                break;
            case R.id.ll_introduce:
                if (ClickUtil.isNotFirstClick()) {
                    return;
                }
                UiHelper.startWebAcyAcy(this, getString(R.string.tip213) + Constant.TYPE_NBO, RequestHost.WebUrls.introduceNbo);
                break;
            case R.id.iv_invite:
                if (ClickUtil.isNotFirstClick()) {
                    return;
                }
                UiHelper.startShareNboAcy(this, Constant.TYPE_NBO);
                break;
        }
    }
    ...
    private void activeNbo() {
        hud.show();
        iRequest.enableNboMine(Constant.TYPE_NBO, DaoUtil.selectNowWallet().getPk(), Util.getDeviceOnlyNum(this), DaoUtil.selectNowWallet().getAddress());
    }
}
