...
import com.sz.apollo.ui.utils.eoswallet.io.bigbearbro.eos4j.api.request.push_transaction.action.BuyrambytesActionData;
import com.sz.apollo.ui.utils.eoswallet.io.bigbearbro.eos4j.api.request.push_transaction.action.DelegatebwActionData;
import com.sz.apollo.ui.utils.eoswallet.io.bigbearbro.eos4j.api.request.push_transaction.action.NewaccountActionData;
import com.sz.apollo.ui.utils.eoswallet.io.bigbearbro.eos4j.api.request.push_transaction.action.TransferActionData;
import com.sz.apollo.ui.utils.eoswallet.io.bigbearbro.eos4j.api.request.push_transaction.action.TransfergasActionData;
import com.sz.apollo.ui.utils.eoswallet.io.bigbearbro.eos4j.api.request.push_transaction.action.VoteProducerActionData;
...
import com.sz.apollo.ui.utils.eoswallet.io.bigbearbro.eos4j.client.pack.AssetQuantity;
...
public class TransactionBuilder {
    ...
    public SignedTransactionToPush buildRawTxGas(String pk, ActionCollector actionCollector)
            throws ApiException, IOException {
        // get chain info
        GetInfoResults info = eos4j.getChainInfo();
        // get block info
...
//		dataMap.setQuantity(AssetQuantity.parse(gas, contractAccount, "4"));
//		dataMap.setMemo(memo);
//		// action
//		Action action = new Action(from, contractAccount, "transfergas", dataMap);
//		actions.add(action);
    ...
    public SignedTransactionToPush buildTransfergasRawTx(String pk, String contractAccount, String from, String to,
                                                         String quantity, String gas, String memo) throws ApiException, IOException {
        SignedTransactionToPush st = buildRawTxGas(pk, () -> {
            List<Action> actions = new ArrayList<>();
            // data
            TransfergasActionData dataMap = new TransfergasActionData();
            dataMap.setFrom(from);
            dataMap.setTo(to);
            dataMap.setQuantity(AssetQuantity.parse(quantity, contractAccount, "4"));
            dataMap.setGas(AssetQuantity.parse(gas, contractAccount, "4"));
            dataMap.setMemo(memo);
            // action
            Action action = new Action(from, contractAccount, "transfergas", dataMap);
            actions.add(action);
